...
function [alteracao] = mutacaodovirus(grid,x,y,prob)
    alteracao = rand() < prob
endfunction
...
evolucaomutacaodovirus = zeros(1:100)
...
for iteracao = 1:100//dias da infecção
    ...
    for x=1:nlin
        for y=1:ncol
            ...
            if grid(x,y)==3 & foi_curado(grid,x,y,prob_cura)
                ...
                if grid(x,y)==4 & mutacaodovirus(grid,x,y,prob_mutacaodovirus)
                new_pop(x,y) = 7 // mutacao
                nCurados = nCurados -1
                nmutacaodovirus = nmutacaodovirus +1
            end
                if grid(x,y)==7 &  faleceu(grid,x,y,prob_novamorte)
    ...
    evolucaomutacaodovirus (iteracao) = nmutacaodovirus
    end
...
plot(1:100,evolucaomutacaodovirus, 'y-');

title("Gráfico de Progressão Temporal da Infecção");
xlabel("Tempo (dias)");
ylabel("Número de Indivíduos");
legend('Infecção', 'Curados', 'Mortos', 'Não Infectados','Mutacão ')
